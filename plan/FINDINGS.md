...
| O-32 | ~~**`startLoop` `pauseOnHidden` auto-resume tramples app-managed pause**~~ — **fixed 2026-07-16**: visibility handler only resumes a pause the driver initiated; app-managed `world.pause()` survives tab re-show. Tests in `lifecycle.test.ts`. | Prism, Vite template (2026-06-10 v1.0 repair) | B | Closed. |
| O-33 | **No browser-importable build for no-build consumers** — workspace `exports` still point at TypeScript source for monorepo DX; **npm-published packages already resolve to `dist/` via `publishConfig`** (verified on registry 1.0.0). Remaining gap: import-map / CDN docs + cold-install CI from empty repo (PLAN WS-2). | Halls (2026-06-10 audit) | C/v0.2 | Packaging; pairs with the `create-domecs` CLI item in §3. |
