...
| O-3 | **Pause-gating boundary** — `tick`/`fixed` are gated off at `setScale(0)`, so control input (resume/save/build hotkeys) read inside a `tick` system goes dead while paused; the working pattern is reading from the always-fires `tickStart` signal. Document which hooks fire at scale 0 vs not; consider an opt-in `runWhilePaused` system flag for pure UI/control logic. Lyceum re-hit this independently: a `tick`-schedule control system could pause but never resume (the unpause key is invisible once schedules freeze); the working pattern was moving pause/speed entirely outside the world into a plain DOM keydown listener calling `pause()/resume()/setScale()` + `requestTick()`. | Railroad, Lyceum | B | The signal-vs-system split is correct but undocumented and easy to get wrong. |
...
| O-34 | **`spawn()` shallow-copies component values — undocumented divergence trap** — `world.spawn([entry(C, v)])` copies `v`'s top-level fields into the store, so the caller's local object silently stops tracking the live component for *scalar* fields, while *nested array/object refs stay shared* and keep mutating live state. Lyceum hit the worst version: a test asserting on the local `PlantState` passed for buffer-array mutations (shared ref) but failed for `stage` (diverged scalar) — same object, half live, half stale. Correct pattern: capture the entity id and read via `world.getComponent(id, C)`. Document the copy semantics on `spawn`/`entry`; consider a dev-mode warning or deep-freeze of the input bag. | Lyceum (2026-06-10 MVP build) | B | `world.ts` spawn path; the half-shared/half-diverged failure mode is uniquely confusing to debug. |
| O-35 | **One-shot `keyDelta` edges are invisible to low-rate `fixed` systems** — `keyDelta.pressed/released` live for exactly one render tick, so a `fixed` system at 4 Hz sampling them catches ~1/15 of presses at 60 fps (Lyceum's interact keys appeared "randomly dead"; held-key `keys` reads worked fine, which masked the cause). The forced shape: all edge-triggered input handling moves to the `tick` schedule, which drags cooldown bookkeeping into scaled-seconds (`ctx.time.scaledDelta`) even when the rest of the sim is tick-counted. Want either per-schedule edge latching (deliver each press once *per consuming schedule*) or a prominent docs warning that `keyDelta` is render-tick-scoped and `fixed` systems must not read it. | Lyceum (2026-06-10 MVP build) | B | `@domecs/input` collector + `time.ts`; pairs with O-3's "which hooks see what" doc gap. |
...
| O-37 | **`stepOnce()` never fires `fixed` systems** — repeated `stepOnce()` calls in headless tests advance `tick` systems but the fixed accumulator never crosses a step boundary, so a world whose whole sim lives on the `fixed` schedule appears completely inert under the API whose name suggests "advance the world once". Lyceum's headless tests had to call `step(fixedStep)` explicitly with the exact step duration. Document the `stepOnce` vs `step(dt)` vs fixed-accumulator relationship in SPEC (what dt `stepOnce` uses, why fixed may not fire), or give `stepOnce` an explicit `{ dt }` option. | Lyceum (2026-06-10 MVP build) | B | Headless testing is the advertised workflow; this is the first wall every fixed-schedule test hits. |
